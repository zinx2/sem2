...
public:
	...
	explicit FormBorrow(int width, int height, QWidget *parent = 0) : CPDialog(width, height, parent)
	{
		...
		layout()->addWidget(
			...
			->append(new CPLabel(35, 25, kr("대출자")))
			->append(edNameUserOrAdmin)
			->append(btnEmployee)
			->append(new CPLabel(55, 25, kr("대출일")))
	...
	void confirm()
	{
		...
		QString strDate = kr("대출날짜 : ") + edDateBorrowedOrReturned->text() + "\n";
		QString strNameUser = kr("대출자 : ") + edNameUserOrAdmin->text() + "\n";
