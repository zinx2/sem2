...
public:
    CPJoin(QWidget* parent = 0) : QWidget(parent)
    {
        ...
        m_row1->append((new CPLabel(20, 20))->initImage(":/imgs/circle.png"))->append((new CPLabel(150, 20, kr("회원관리")))->initAlignment(Qt::AlignLeft|Qt::AlignVCenter));
        ...
        m_lbCheckPass = (new CPLabel(200, 20, kr("비밀번호를 입력하세요.")))->initAlignment(Qt::AlignRight|Qt::AlignVCenter)->initStyleSheet("color:red")->initFontSize(10);
        CPWidget* wdEdPass = (new CPWidget(200, 65, new QVBoxLayout))->initSpacing(5)->append(m_edPassRe)->append(m_lbCheckPass);

        m_row3->append((new CPLabel(70, 115, kr("비밀번호")))->initAlignment(Qt::AlignRight|Qt::AlignTop))
        ...
        m_row4->append((new CPLabel(70, 40, kr("직원찾기")))->initAlignment(Qt::AlignRight|Qt::AlignVCenter))->append(m_edEmp)->append(m_btnEmp);

        m_row5 = (new CPWidget(width(), 40, new QHBoxLayout))->initAlignment(Qt::AlignLeft | Qt::AlignVCenter)->initSpacing(10);
        m_row5->append((new CPLabel(70, 40, kr("부서찾기")))->initAlignment(Qt::AlignRight|Qt::AlignVCenter))->append(m_edPart)->append(m_btnPart);

        m_row6 = (new CPWidget(width(), 85, new QHBoxLayout))->initAlignment(Qt::AlignRight | Qt::AlignBottom)->initSpacing(10);
