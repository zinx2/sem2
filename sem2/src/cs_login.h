...
public:
    CPLogin() : CPDialog("", 400, 280)
    {
        ...
        m_btnClose = (new Command("close", "X", 40, 30))->initFontSize(20)
                ->initEffect(p->btnReleasedStyleNavy, p->btnHoveredStyleNavy, p->btnSelectedStyleNavy);
        m_btnLogin = (new Command("login", kr("로그인"), 90, 90))->initFontSize(20)
                ->initEffect(p->btnReleasedStyleNavy, p->btnHoveredStyleNavy, p->btnSelectedStyleNavy);
        ...
        m_row3 = (new CPWidget(width(), 20, new QHBoxLayout))->initAlignment(Qt::AlignLeft | Qt::AlignVCenter)
                ->initContentsMargins(40, 0, 0, 0);

        m_row3->append((new CPLabel(m_row3->width(), m_row3->height(), kr("온라인평생교육원 자산 관리 시스템")))
                    ->initAlignment(Qt::AlignLeft|Qt::AlignVCenter)->initFontSize(20));
        layout()->addWidget(m_row3);

        m_row4 = (new CPWidget(300, 90, new QHBoxLayout))->initAlignment(Qt::AlignLeft | Qt::AlignTop)
