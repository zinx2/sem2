...
#include "cs_style.h"
#include "cs_model.h"
class SelectorEmployee : public CPDialog
{
	Q_OBJECT
public:
	explicit SelectorEmployee(QString title, int width, int height, QWidget* parent = 0) : CPDialog(title, width, height, parent)
	{
		...
		m_wdTail->layout()->addWidget((new CPLabel(m_width - 150, 25, kr("직원을 선택하세요.")))->initAlignment(Qt::AlignCenter));
...
private:
	...
	void updateTable()
	{
		...
		m_table->setFixedHeight(m_height - m_wdTail->height());
		m_table->setColumnWidth(0, 50);
		m_table->setColumnWidth(1, 300 - m_table->verticalScrollBar()->width());
